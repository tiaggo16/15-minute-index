...
# Set up folium map
map1 = folium.Map(
    tiles="Stamen Toner", location=([37.738684, -122.450523]), zoom_start=12
)

# Set up the apartment dictionary with real coordinates
apartments = {
    "first": {"location": [-122.430954, 37.792965]},
...
for name, apt in apartments.items():
    ...
    for typ, category in categories_poi.items():
        ...
        apt["categories"][typ]["geojson"] = ors.places(**params_poi)[
            "features"
        ]  # Actual POI request
...
for name, apt in apartments.items():
    apt["shortest_sum"] = sum(
        [min(cat["durations"]) for cat in apt["categories"].values()]
    )
